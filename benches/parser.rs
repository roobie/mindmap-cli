...
use mindmap_cli::{parse_node_line, Mindmap};
use std::io::Cursor;

fn bench_parse_node_line(c: &mut Criterion) {
    let line = "[123] **AE: Example Node** - This is a description with [456] and [789] references.";
...
[3] **AE: Third** - Description three"#;
    c.bench_function("mindmap_from_string", |b| {
        b.iter(|| Mindmap::load_from_reader(black_box(Cursor::new(content)), std::path::PathBuf::from("-")))
    });
}
