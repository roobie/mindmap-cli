...
mod tests {
    ...
    #[test]
    fn test_load_from_reader_and_no_save_on_stdin() -> Result<()> {
        let content = "[1] **AE: A** - base\n[2] **AE: B** - refers [1]\n";
        // load from a string reader and path '-' to simulate stdin
        let mm = Mindmap::load_from_reader(content.as_bytes(), PathBuf::from("-"))?;
        assert_eq!(mm.nodes.len(), 2);
        // saving should fail because path == '-'
        let err = mm.save().unwrap_err();
        assert!(format!("{}", err).contains("loaded from stdin"));
        Ok(())
    }
}
