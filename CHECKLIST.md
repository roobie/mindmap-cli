...
3. Implement Node and Mindmap structs
   - Node { id: u32, raw_title: String, body: String, references: Vec<u32>, line_index: usize }
...
4. Implement loader Mindmap::load(path: PathBuf) -> Result<Mindmap>
   ...
   - Parse references from body using ref regex (\[(\d+)\]).
...
8. Implement cmd_list --type / --grep
   ...
   - Acceptance: list with type AE returns only AE nodes; grep filters title/body.
...
14. Implement cmd_add --type --title --body
...
15. Implement cmd_edit id (opens $EDITOR)
    ...
    - Acceptance: editing title/body succeeds; changing ID is rejected.
...
17. Implement cmd_verify id
    - Append "(verify YYYY-MM-DD)" to body if not present (use chrono for date).
    - Update mm.lines and save.

18. Implement put & patch (HTTP semantics)
    - put <id> --line "[id] **TYPE: Title** - body [N]"  # full-line replace, id must match
    - patch <id> [--type TYPE] [--title TITLE] [--body DESC] [--strict]
...
- deprecate/verify update the node title/body and are idempotent-safe
